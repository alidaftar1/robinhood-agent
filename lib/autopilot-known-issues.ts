...
export const KNOWN_ISSUES: KnownIssue[] = [
  ...
  {
    date: "2026-07-08",
    title: "Drop-check risk-exit + redeploy mis-flagged as a thesis/execution contradiction",
    lesson:
      "On 2026-07-08 an intraday /api/drop-check?scope=influencer run stopped out MSTR (an influencer pick that hit its −5% leash, filled ~$92.93 vs $98.54 buy) and redeployed the freed cash into ONE new buy, SCHW. mergeRunsByDate/preferRun kept the 7:30 run (chosen for its computed return + more trades) as the canonical record and unioned in those two trades (MSTR sell + SCHW buy), but the merged run carries ONLY the morning summary — the drop-check's own [RISK-EXIT] summary is discarded. The skeptical reviewer then saw SCHW/MSTR in the trades array but NOT in the morning thesis/TRADE_DECISION (which had explicitly said 'skip SCHW — XLF, over cap' and 'HOLD MSTR'), and raised TWO HIGH 'internal contradiction' concerns plus two related mediums (MSTR held-then-sold; SCHW cash diff). All four were false positives — those trades legitimately came from the later risk-exit run. Same family as #7 (two full runs in one day), but that entry's check is about positions/cash reconciliation, not the thesis-vs-trades divergence that produces these reviewer false positives. Residual note: the merged trade record shows the drop-check SELL at the model's reported/estimate price (MSTR $98.54 = its own buy price) rather than the live fill ($92.93) — cosmetic here (return −2.45%, impliedTransfer $20.35 both within noise), but the P&L on such a sell row can read wrong.",
    check:
      "Before flagging a trade as a thesis/execution contradiction, check whether it is the signature of an intraday drop-check risk-exit + redeploy: an influencer-name SELL at roughly −5% from its buy price (or a +40% take-profit), optionally paired with exactly ONE new BUY, where the sold/bought symbols are ABSENT from the morning TRADE_DECISION and appear as state 'submitted' alongside the morning 'filled' trades. That divergence is EXPECTED — the merged run keeps only the morning summary — so treat it as a normal intraday exit, NOT a logic failure or a decision/execution mismatch. Do, however, verify the redeployment BUY still respects the 40% sector SOFT cap: the intraday redeploy runs a fresh model call that can re-concentrate a sector the MORNING run had just deliberately trimmed (07-08 SCHW pushed XLF back to ~42% right after the morning bought ELV/XRAY specifically to avoid adding financials). If so, flag the sector re-concentration (a real, lower-severity observation), not the phantom contradiction.",
  },
];
